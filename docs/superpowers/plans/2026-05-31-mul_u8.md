# mul_u8 Kernel Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement a pointwise CUDA kernel that performs bitwise element-wise multiplication on the lowest bit of two `u8` tensors, outputting a bit-shifted `u8` result and a templated `float` or `half` result.

**Architecture:** A vectorized 1D CUDA kernel where each thread processes 4 elements utilizing `uchar4`, `float4`, and `half2` to maximize memory bandwidth. Includes a CPU reference implementation for validation.

**Tech Stack:** C++, CUDA

---

### Task 1: Setup and CPU Reference Implementation

**Files:**
- Create: `op/mul/mul_u8.cu`

- [ ] **Step 1: Create file and write CPU reference function**
...
```bash
git add op/mul/mul_u8.cu
git commit -m "feat: add cpu reference for mul_u8"
```

### Task 2: Implement CUDA Kernel

**Files:**
- Modify: `op/mul/mul_u8.cu`

- [ ] **Step 1: Write the Vectorized CUDA Kernel**

Append this code to `op/mul/mul_u8.cu`:
...
```bash
git add op/mul/mul_u8.cu
git commit -m "feat: implement vectorized mul_u8 kernel"
```

### Task 3: Implement Testing and Profiling Logic

**Files:**
- Modify: `op/mul/mul_u8.cu`

- [ ] **Step 1: Write test function and main**
...
Compile and run the code to ensure tests pass:
```bash
nvcc -O3 -arch=sm_80 op/mul/mul_u8.cu -o op/mul/mul_u8_test
./op/mul/mul_u8_test
```
*(Note: Use appropriate architecture flag for the current GPU, e.g., sm_80 or sm_89)*

- [ ] **Step 3: Commit**

```bash
git add op/mul/mul_u8.cu
git commit -m "test: add and run tests for mul_u8 kernel"
```
