...
- fused 与 unfused 数值语义；
- forward 输出、输入梯度及所有参数梯度；
- Autograd 多次调用和必要的梯度累积语义。
...
backward-only 可以复用预建计算图，但必须注明 `retain_graph`、grad 清零和输入复用方式。直接 forward+backward 必须每次真实建图和反传；端到端加速比只使用直接测量值，不得用 forward 与 backward-only 相加。

统一是否包含 tensor 分配、metadata 构建、权重预处理、layout 转换和 Autograd wrapper。性能循环内不得隐式触发重新编译或新的 Autotune 配置搜索。
...
- fused 前后 launch 数和中间 tensor 流量；
...
优先按证据依次考虑：算法与 fusion、内存流量、program/tile 映射、layout、`num_warps`、`num_stages`、持久化策略和较小参数搜索。
