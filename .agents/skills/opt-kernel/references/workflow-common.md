...
- 目标实现与 fused/unfused、forward/backward 路径的边界。

### 检查环境和 GPU

先查询 GPU 名称、compute capability、利用率和显存占用。使用 benchmark 实际运行的 device；多 GPU 或多架构环境不得默认选择。
...
先收集低开销概览，再按假设收集详细指标，避免每轮无条件使用 full profile。记录 profiler 版本、kernel 筛选方式、launch 位置、输入配置和重放影响。
...
根据证据生成有优先级的实验。每个实验写明假设、单一主要变量、预期、风险和验收条件。
...
每轮只验证一个主要假设；保持接口和语义时，允许完整重构数据流、映射和存储层级。
...
| 某类配置提升至少 5% 且至少为噪声上界 3 倍，可可靠选择 | `KEEP` 条件路径 |
...
完整读取并使用 [优化报告模板](report-template.md)。按最终决策重组报告，不追加阶段流水账。
