...
使用独立同语义 reference，覆盖对齐、非对齐、尾部、stride/layout、dtype 和边界输入。
...
优先验证算法与数据流，再实验 tile/layout、并行映射、pipeline/stage、缓存和编译参数。每个参数实验保持单一主要假设，并在业务及边界 shape 上验证，避免只对固定输入过拟合。
