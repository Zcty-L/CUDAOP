...
修改 thread/warp/tile 映射、shared memory 布局或向量化访存时，先做实验有效性审计：写出代表性 lane/warp 到逻辑坐标和实际字节地址的映射；检查 global memory 的合并、对齐和逻辑流量；
按目标架构的 bank 数与 bank width 计算 shared load/store 的 broadcast、bank conflict 和事务量；同时记录 padding、swizzle、transpose、额外同步、寄存器与 occupancy 代价。优先用定向 NCU 指标验证静态计算。
若候选实现引入可避免的次生问题，例如新增 bank conflict、非合并访问、重复流量、spill 或 occupancy 崩塌，则该结果只否定当前实现，不能否定原始优化方向。将可行的 padding、swizzle、线程重排或协作加载修正版作为同一父假设的后续单变量实验；
只有修正版已经实测，或已用具体证据证明修正成本必然覆盖收益，才能关闭该方向。
...
连续两个实验没有提升时只重新分析瓶颈和优先级，不得直接停止；将重新分析得到的可信方向加入实验计划并继续执行。
进入阶段 5 前必须建立并清空“候选方向闭环清单”。将在本轮分析、计划、进度更新或回复中由自己表述为“值得尝试”“可以尝试”“建议尝试”“可检查”“可进行”“下一步”“高优先级”或其他同等含义的方向全部加入清单，并逐项满足以下一种状态：
- `VERIFIED`：已实际实现，完成实验有效性审计、正确性与性能测试，不存在尚未验证的可修正混杂因素，并按决策规则记录 `KEEP`、`REVERT` 或 `CRASH`；
- `EXCLUDED`：实现前已由 profiler、生成代码、硬件/架构约束、数学上界或现有等价实验的具体证据排除，并记录证据；
- `BLOCKED`：需要改变语义、扩大用户授权范围、缺少关键输入，或环境无法形成可信测量，并明确向用户报告阻塞条件。

仅分析可行性、列出建议、降低优先级，或写入“后续可尝试”“剩余方向”不构成闭环。清单中存在任何未闭环方向时，不得停止实验循环，不得进入阶段 5/6，也不得输出 `[NO_GAIN]`。除非用户明确限制实验方向或要求提前停止，仅在满足以下任一条件时停止：
- 候选方向闭环清单已清空，所有高优先级假设均已验证或排除，且每个主要瓶颈至少验证过一个结构性方案；
- 已用硬件吞吐、有效带宽、指令利用率或同语义参考实现量化证明达到合理上限，所有剩余可信路径均已闭环；
- 所有未完成方向均为已报告的 `BLOCKED`，当前环境或授权范围内无法继续形成可信决策。
