...
name: opt-kernel
description: >-
  分析并迭代优化使用 CUDA C++、Triton 或 cuTile 实现的 GPU kernel，完成目标定位、环境检查、基线冻结、瓶颈定位、单变量实验、正确性与性能验证及报告交付。
  用于 $opt-kernel、CUDA/Triton/cuTile kernel 调优、GPU 算子性能分析、Nsight Compute 分析或要求优化 op/{name} 下算子的任务。
---
