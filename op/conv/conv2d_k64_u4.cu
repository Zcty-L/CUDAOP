...
#pragma unroll
    ...
    auto load_input = [&](int k_iter, int buf)
    {
        ...
        if (global_k < in_features)
        {
            ...
            {
                // ---- 1×1 fast path ----
                // ky=kx=0 (no spatial kernel), ih=oh*Sh-Ph, iw=ow*Sw-Pw
                // For s=1/p=0: H_out=H, W_out=W → ih/iw always in [0,H)/[0,W)
                int c_idx = global_k;
#pragma unroll
                for (int b = 0; b < 4; b++)
                {
            ...
            {
                // ---- 3×3 path ----
                // ky,kx from linearized K-index; compiler folds /9, %9, %3
...
#pragma unroll
    ...
    // ----------------------------------------------------------------
    // Double-buffer prologue: preload buffer 0
    // ----------------------------------------------------------------
    if (k_iters > 0)
    {
        load_weight(0, 0);
    ...
    // ----------------------------------------------------------------
    // K-loop: compute k while cp.async.ca loads k+1
    // ----------------------------------------------------------------
