...
// FP8 E4M3 输入、FP16 累加的 CuTe GEMM。测试显式覆盖
// FP32 -> FP8 tensor-wide 量化、量化域 Tensor Core GEMM、
// FP16 accumulator -> FP32 反量化，以及 Block Swizzle8 CTA 映射。
...
constexpr const char *kIdentityName = "FP8/FP16-Identity";
constexpr const char *kBlockSwizzleName = "FP8/FP16-BlockSwizzle8";
constexpr const char *kCublasName = "cuBLASLt FP8/FP32";

// 生成值位于 [-0.5, 0.5)。若按 E4M3 最大有限值 448 做满量程量化，
// 量化域乘积会让 K=8192 的 FP16 accumulator 很快溢出。这里把量化值
// 的目标绝对值限制为 4，在量化精度和 FP16 累加余量间做平衡。
constexpr float kInputAbsoluteMaximum = 0.5F;
constexpr float kQuantizedAbsoluteTarget = 4.0F;
...
// cuBLASLt 的 FP8 路径按规范使用 FP32 累加；CuTe 路径按本测试要求
// 使用 FP16 累加，所以二者不应逐 bit 相同。
constexpr float kAbsoluteTolerance = 3.0e-1F;
constexpr float kRelativeTolerance = 5.0e-2F;
...
public:
        ...
        : workspace_(kWorkspaceBytes)
    {
        ...
        // 令 cuBLASLt 计算 C^T = B * A^T，可同时满足 FP8 kernel 要求的
        ...
        {
            throw std::runtime_error(
                "cuBLASLt found no FP8 E4M3 GEMM algorithm");
        }
    ...
    float scale)
{
    ...
    {
        // tensor-wide 对称量化：q = FP8(x / scale)。这里的数据生成器
...
void cute_gemm_fp8_fp16_kernel(
    ...
    float dequant_scale)
{
    ...
    // 每个线程每次搬运连续 16 个 FP8（16B）。partition_S/D 把完整
    ...
    // SM89_16x8x32_F16E4M3E4M3F16_TN 令 A/B fragment 为 E4M3，
    // make_fragment_C 得到 FP16 寄存器累加器。
    ...
    // tile 的 cp.async G2S 可以与当前 tile 的 S2R/QMMA 跨 stage 重叠。
    CUTE_NO_UNROLL
    ...
    // MMA 的结果仍处于量化域。这里先把 FP16 accumulator 转为 FP32，
    // 再乘 scale_A * scale_B 反量化，最终输出真实数值域的 FP32 C。
    CUTE_UNROLL
    ...
    BlockRasterMode block_raster_mode)
{
    ...
    // FP8 元素占 1B。K tile 每行正好 128B，Swizzle<3,4,3>
    ...
    // 128 个线程按 16x8 排列，每线程每条 cp.async 搬运 16 个 FP8=16B。
    ...
    // 指令语义为 D(FP16) = A(E4M3) * B(E4M3) + C(FP16)。2x2 个
    ...
    const auto kernel = cute_gemm_fp8_fp16_kernel<
...
double cpu_reference_value(int row, int column, int k)
{
    ...
    return result * static_cast<double>(kDequantScale);
}
...
int main(int argc, char **argv)
{
    try
    {
        ...
        std::cout << "CuTe FP8 E4M3 input / FP16 accumulate GEMM "
        ...
        std::cout << "  " << std::left << std::setw(30)
                  << "Quantization" << "q = E4M3(x / scale)\n";
        std::cout << "  " << std::left << std::setw(30)
                  << "Scale A / B" << kScaleA << " / " << kScaleB << '\n';
        std::cout << "  " << std::left << std::setw(30)
                  << "Quantized amax target" << kQuantizedAbsoluteTarget
                  << " (FP16 accumulation headroom)\n";
        std::cout << "  " << std::left << std::setw(30)
                  << "Dequantization"
                  << "FP32(acc_fp16) * scale_A * scale_B\n";
        std::cout << "  " << std::left << std::setw(30)
        ...
        std::cout << "  " << std::left << std::setw(30)
                  << "MMA" << "m16n8k32 E4M3 inputs / FP16 accumulate\n";
        std::cout << "  " << std::left << std::setw(30)
                  << "Block raster"
        ...
        {
            throw std::runtime_error(
                "FP8 input / FP16 accumulate GEMM correctness failed");
        }
        ...
        std::cout << "[SUCCESS] FP8 E4M3 quantization, FP16 accumulation, "
