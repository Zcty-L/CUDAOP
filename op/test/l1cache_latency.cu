#include <algorithm>
#include <array>
#include <cstdint>
#include <cstdlib>
#include <iomanip>
#include <iostream>
#include <stdexcept>
#include <string>

#include <cuda_runtime.h>

#include "ptx_utils.cuh"

namespace
{

constexpr int kThreads = 4;
constexpr int kWarmupLaunches = 100;
constexpr int kLoadIterations = 50;

void check_cuda(cudaError_t status, const char *operation)
{
    if (status != cudaSuccess)
    {
        throw std::runtime_error(
            std::string(operation) + ": " + cudaGetErrorString(status));
    }
}

__global__ __launch_bounds__(kThreads, 1)
void l1cache_latency_kernel(
    const uint64_t *pointer_chain,
    uint32_t *valid,
    uint64_t *clocks)
{
    const uint64_t initial_address =
        reinterpret_cast<uint64_t>(
            pointer_chain + threadIdx.x);
    uint64_t load_address = initial_address;

    // Warm this thread's self-referencing pointer into the unified L1/TEX.
    #pragma unroll
    for (int i = 0; i < kLoadIterations; ++i)
    {
        ptx::ldg64_nc(
            load_address,
            reinterpret_cast<const void *>(load_address));
    }

    __syncthreads();
    const uint64_t start = ptx::read_clock64();

    #pragma unroll
    for (int i = 0; i < kLoadIterations; ++i)
    {
        // Pointer chasing prevents independent loads from hiding latency.
        ptx::ldg64_nc(
            load_address,
            reinterpret_cast<const void *>(load_address));
    }

    const uint64_t stop = ptx::read_clock64();

    clocks[threadIdx.x] = stop - start;
    valid[threadIdx.x] = load_address == initial_address;
}

}  // namespace

int main()
{
    uint64_t *device_pointer_chain = nullptr;
    uint32_t *device_valid = nullptr;
    uint64_t *device_clocks = nullptr;

    try
    {
        int device = 0;
        cudaDeviceProp properties{};
        check_cuda(cudaGetDevice(&device), "cudaGetDevice");
        check_cuda(
            cudaGetDeviceProperties(&properties, device),
            "cudaGetDeviceProperties");

        std::cout << "L1 cache latency test\n\n";
        std::cout << "[Configuration]\n";
        std::cout << "  " << std::left << std::setw(24)
                  << "GPU" << properties.name << '\n';
        std::cout << "  " << std::left << std::setw(24)
                  << "Compute capability"
                  << properties.major << '.' << properties.minor << '\n';
        std::cout << "  " << std::left << std::setw(24)
                  << "Active threads" << kThreads << '\n';
        std::cout << "  " << std::left << std::setw(24)
                  << "Dependent loads" << kLoadIterations << '\n';
        std::cout << "  " << std::left << std::setw(24)
                  << "Pre-timing L1 loads" << kLoadIterations << '\n';
        std::cout << "  " << std::left << std::setw(24)
                  << "Warmup launches" << kWarmupLaunches << '\n';
        std::cout << "  " << std::left << std::setw(24)
                  << "Pointer-chain size"
                  << kThreads * sizeof(uint64_t) << " bytes\n\n";

        std::cout << "[Stage 1/3] Allocating self-referencing pointers\n";
        check_cuda(
            cudaMalloc(
                &device_pointer_chain,
                kThreads * sizeof(uint64_t)),
            "cudaMalloc(device_pointer_chain)");
        check_cuda(
            cudaMalloc(
                &device_valid,
                kThreads * sizeof(uint32_t)),
            "cudaMalloc(device_valid)");
        check_cuda(
            cudaMalloc(
                &device_clocks,
                kThreads * sizeof(uint64_t)),
            "cudaMalloc(device_clocks)");

        std::array<uint64_t, kThreads> host_pointer_chain{};
        for (int i = 0; i < kThreads; ++i)
        {
            host_pointer_chain[i] =
                reinterpret_cast<uint64_t>(
                    device_pointer_chain + i);
        }
        check_cuda(
            cudaMemcpy(
                device_pointer_chain,
                host_pointer_chain.data(),
                host_pointer_chain.size() * sizeof(uint64_t),
                cudaMemcpyHostToDevice),
            "cudaMemcpy(device_pointer_chain)");

        std::cout << "\n[Stage 2/3] Warming instruction and L1 caches\n";
        for (int i = 0; i < kWarmupLaunches; ++i)
        {
            l1cache_latency_kernel<<<1, kThreads>>>(
                device_pointer_chain,
                device_valid,
                device_clocks);
        }
        check_cuda(cudaGetLastError(), "warmup kernel launch");
        check_cuda(cudaDeviceSynchronize(), "warmup synchronization");

        std::cout << "\n[Stage 3/3] Measuring warm L1 loads\n";
        l1cache_latency_kernel<<<1, kThreads>>>(
            device_pointer_chain,
            device_valid,
            device_clocks);
        check_cuda(cudaGetLastError(), "benchmark kernel launch");
        check_cuda(cudaDeviceSynchronize(), "benchmark synchronization");

        std::array<uint32_t, kThreads> host_valid{};
        std::array<uint64_t, kThreads> host_clocks{};
        check_cuda(
            cudaMemcpy(
                host_valid.data(),
                device_valid,
                host_valid.size() * sizeof(uint32_t),
                cudaMemcpyDeviceToHost),
            "cudaMemcpy(host_valid)");
        check_cuda(
            cudaMemcpy(
                host_clocks.data(),
                device_clocks,
                host_clocks.size() * sizeof(uint64_t),
                cudaMemcpyDeviceToHost),
            "cudaMemcpy(host_clocks)");

        const bool pointers_valid = std::all_of(
            host_valid.begin(),
            host_valid.end(),
            [](uint32_t value)
            {
                return value == 1;
            });
        const bool clocks_valid = std::all_of(
            host_clocks.begin(),
            host_clocks.end(),
            [](uint64_t value)
            {
                return value > 0;
            });
        if (!pointers_valid || !clocks_valid)
        {
            throw std::runtime_error(
                "L1 pointer-chain validation failed");
        }

        const auto [minimum, maximum] = std::minmax_element(
            host_clocks.begin(),
            host_clocks.end());
        uint64_t total = 0;
        for (uint64_t clocks : host_clocks)
        {
            total += clocks;
        }

        const double minimum_latency =
            static_cast<double>(*minimum) / kLoadIterations;
        const double average_latency =
            static_cast<double>(total) /
            (kThreads * kLoadIterations);
        const double maximum_latency =
            static_cast<double>(*maximum) / kLoadIterations;

        std::cout << "\n[Result]\n";
        std::cout << std::fixed << std::setprecision(2);
        std::cout << "  " << std::left << std::setw(24)
                  << "Minimum latency" << minimum_latency
                  << " cycles/load\n";
        std::cout << "  " << std::left << std::setw(24)
                  << "Average latency" << average_latency
                  << " cycles/load\n";
        std::cout << "  " << std::left << std::setw(24)
                  << "Maximum latency" << maximum_latency
                  << " cycles/load\n";
        std::cout << "\n[SUCCESS] L1 cache latency test passed\n";

        check_cuda(
            cudaFree(device_clocks),
            "cudaFree(device_clocks)");
        device_clocks = nullptr;
        check_cuda(
            cudaFree(device_valid),
            "cudaFree(device_valid)");
        device_valid = nullptr;
        check_cuda(
            cudaFree(device_pointer_chain),
            "cudaFree(device_pointer_chain)");
        device_pointer_chain = nullptr;
    }
    catch (const std::exception &error)
    {
        if (device_clocks != nullptr)
        {
            cudaFree(device_clocks);
        }
        if (device_valid != nullptr)
        {
            cudaFree(device_valid);
        }
        if (device_pointer_chain != nullptr)
        {
            cudaFree(device_pointer_chain);
        }
        std::cout << "\n[FAILED] " << error.what() << '\n';
        return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}
